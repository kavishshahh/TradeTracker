...
try:
    ...
    # For development without Firebase, we'll use mock data
    db = None
...
@app.get("/")
async def root():
    return {"message": "TradeTracker API is running"}
...
async def get_trades(user_id: str, from_date: Optional[str] = None, to_date: Optional[str] = None):
    try:
        if db:
            ...
            if from_date:
                trades_ref = trades_ref.where('date', '>=', from_date)
            if to_date:
                trades_ref = trades_ref.where('date', '<=', to_date)
